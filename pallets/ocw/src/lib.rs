...
pub mod pallet {
    ...
    #[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug)]
    pub struct PriceInfo<Public> {
        price_u: u64,
        price_f: Permill,
        public: Public,
    }

    impl<T: SigningTypes> SignedPayload<T> for PriceInfo<T::Public> {
        fn public(&self) -> T::Public {
            self.public.clone()
        }
    }
    ...
    pub enum Error<T> {
        ...
        // Error returned when submiting price info to chain
        SubmitPriceToChainError,
    }
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        -> TransactionValidity
        {
            ...
            match call {
                ...
                Call::submit_price_unsigned_with_signed_info(ref payload, ref signature) => {
                    if !SignedPayload::<T>::verify::<T::AuthorityId>(payload, signature.clone()) {
                        return InvalidTransaction::BadProof.into();
                    }
                    valid_tx(b"submit_price_unsigned_with_signed_info".to_vec())
                },
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight(10000)]
        pub fn submit_price_unsigned_with_signed_info(origin: OriginFor<T>, info: PriceInfo<T::Public>,
            _signature: T::Signature) -> DispatchResult
        {
            let _ = ensure_none(origin)?;
            // we don't need to verify the signature here because it has been verified in
            //   `validate_unsigned` function when sending out the unsigned tx.
            let PriceInfo { price_u, price_f, public } = info;
            log::info!("submit_price_unsigned_with_signed_info: ({}, {:?})", price_u, public);
            Self::append_or_replace_price(price_u, price_f);

            Self::deposit_event(Event::NewPrice(None, price_u, price_f));
            Ok(())
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// Append a new price to the tail of the list, removing an element from the head if reaching
        ///   the bounded length.
        fn append_or_replace_price(price_u: u64, price_f: Permill) {
            Prices::<T>::mutate(|prices| {
                if prices.len() == PRICE_VEC_LEN {
                    let _ = prices.pop_front();
                }
                prices.push_back((price_u, price_f));
                log::info!("Price vector: {:?}, {:?}", price_u, price_f);
            });
        }

        /// parse price string to internal price storage type, i.e. (u64,Permill)
        fn parse_price_str(price_str: &str) -> Option<(u64, Permill)> {
            // in case the price is an integer
            let try_digits = price_str.parse::<u64>();
            if try_digits.is_ok() {
                return Some((try_digits.unwrap(), Permill::zero()))
            }
            // case of float
            ...
            let parts_u = parts[0].parse::<u64>().map_or(0, |v| v );
            // as permill's from_parts only accept u32, only retrieve first 6 digits
            // Note: better to round here, maybe use u128 to parse and then round.
            let parts_f = parts[1][..6].parse::<u32>().map_or(0, |v| v );
            Some((parts_u, Permill::from_parts(parts_f)))
        }

        fn fetch_price_info() -> Result<(), Error<T>> {
            // 利用 offchain worker 取出 DOT 当前对 USD 的价格，并把写到一个 Vec 的存储里，
            // 使用 unsigned with signed payload 方式提交到链上
            // WHY:
            // 1. 由于 signed 交易需要支付 gas fee，而价格是公开信息，且更新频率较高，故选择 unsigned 交易
            // 2. 普通 unsigned 交易极易被用于攻击或者带来伪造信息，且价格信息应确保准确，故需要对交易数据进行签名校验
            // 另附 recipe 中的说明 
            // https://substrate.dev/recipes/off-chain-workers/transactions.html#unsigned-transactions-with-signed-payloads
            let source = "https://api.coincap.io/v2/assets/polkadot";
            let resp_bytes = Self::fetch_from_remote(source).map_err(|e| {
                log::error!("fetch_from_coincap error: {:?}", e);
                <Error<T>>::HttpFetchingError
            })?;

            let resp_str = str::from_utf8(&resp_bytes).map_err(|_| <Error<T>>::HttpFetchingError)?;
            // Print out our fetched JSON string
            log::info!("coincap return: {}", resp_str);
            // Refer doc https://docs.serde.rs/serde_json/
            let price_json: Value = serde_json::from_str(&resp_str).map_err(|_| <Error<T>>::HttpFetchingError)?;
            log::info!("price: {}", price_json["data"]["priceUsd"]);
            let price = Self::parse_price_str(price_json["data"]["priceUsd"].as_str().unwrap());
            // For invalid price data, silent skip
            if price.is_some() {
                let price_tuple = price.unwrap();
                let _ = Self::submit_price_to_chain(price_tuple.0, price_tuple.1);
                log::info!("dot price has been submitted: {:?}", price_tuple);
            }
            Ok(())
        }
        ...
        fn submit_price_to_chain(price_u: u64, price_f: Permill) -> Result<(), Error<T>> {
            // Retrieve the signer to sign the payload
            let signer = Signer::<T, T::AuthorityId>::any_account();

            if let Some((_, res)) = signer.send_unsigned_transaction(
                |acct| PriceInfo { price_u, price_f, public: acct.public.clone() },
                Call::submit_price_unsigned_with_signed_info
                ) {
                return res.map_err(|_| {
                    log::error!("Failed in submit_price_to_chain");
                    <Error<T>>::SubmitPriceToChainError
                });
            }
